...
public class Confirmticket extends AppCompatActivity {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                for (int i = 1; i <= 6; i++) {
                    for (int j = 1; j <= 2; j++) {
                        ...
                        if (Objects.requireNonNull(dataSnapshot.child(currentdate).child(busroute).child("L" + i + j).child("isbooked").getValue()).toString().equals("true")) {
                ...
                for (int i = 1; i <= 6; i++) {
                    for (int j = 1; j <= 3; j++) {
                            ...
                            if (Objects.requireNonNull(dataSnapshot.child(currentdate).child(busroute).child("R" + i + j).child("isbooked").getValue()).toString().equals("true")) {
    ...
    private void checkvalidseat(final String selectedseatID, final String busroute) {
        ...
        final String currentdate = new SimpleDateFormat("dd-MM-yyyy", Locale.getDefault()).format(new Date());
        String path = "Booking/" + currentdate + "/"+getIntent().getStringExtra("busnumber")+"/" + selectedseatID + "/isbooked";
        ...
        ValueEventListener listener =new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (snapshot.getValue().toString().equals("null")) {
                    ...
                    ref1.child(currentdate).child(busroute).child(selectedseatID).child("isbooked").setValue(true);
                    ref2.child(username).child("seat").child(currentdate).child("busroute").setValue(busroute);
                    ref2.child(username).child("seat").child(currentdate).child("seat ID").setValue(selectedseatID);
                    ref1.child(currentdate).child(busroute).child(selectedseatID).child("username").setValue(username);
                    isavailable=true;
